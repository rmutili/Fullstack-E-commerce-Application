...
const CartScreen = () => {
  ...
  const [searchParams] = useSearchParams(); // To get the query string from the url
  const qty = searchParams.get("qty"); // To get the quantity from the query string

  const cart = useSelector((state) => state.cart); // To get the cart state from redux store
  const { cartItems } = cart; // To get the cartItems from the cart state
